<?php //Clase para poder validar los usuarios en la base de datos
    ...
    class Usuarios extends Conexion{
        public function loginUsuario($usuario, $password){
            ...
            $sql = "SELECT * FROM helpdesk.t_usuarios 
                    WHERE usuario = '$usuario' AND password = '$password'";
            $respuesta = mysqli_query($conexion, $sql);

            if(mysqli_num_rows($respuesta) > 0){ //Verificacion del registro del usuario en la base de datos
        ...
        public function agregaNuevoUsuario($datos){
            ...
            if($idPersona > 0){
                $sql = "INSERT INTO helpdesk.t_usuarios (id_rol, 
                                                         id_persona,
                                                         usuario, 
                                                         password, 
                                                         ubicacion)
        ...
        public function agregarPersona($datos){
            $conexion = Conexion::conectar();
            $sql = "INSERT INTO helpdesk.t_persona (paterno,
                                                    materno,
                                                    nombre,
                                                    fecha_nacimiento,
                                                    sexo,
            ...
            $query->bind_param("sssssss", $datos['paterno'],
                                          ...
                                          $datos['correo']);
            $respuesta = $query->execute();
            $idPersona = $query->mysqli_insert_id($conexion);
            $query->close();
            return $idPersona;
        }
            ...
            $sql = "SELECT 
                    ...
                    FROM
                        helpdesk.t_usuarios AS usuarios
                            INNER JOIN
                        helpdesk.t_cat_roles AS roles ON usuarios.id_rol = roles.id_rol
                            INNER JOIN
                        helpdesk.t_persona AS persona ON usuarios.id_persona = persona.id_persona
        ...
        public function actualizarUsuario($datos) {
            ...
            if($exitoPersona){
                $sql = "UPDATE helpdesk.t_usuarios SET id_rol = ?,
                                                usuario = ?,
                                                ubicacion = ? 
                        WHERE id_usuario = ?";
                $query = $conexion->prepare($sql);
        ...
        public function actualizarPersona($datos) {
            ...
            $sql = "UPDATE helpdesk.t_persona SET paterno = ?,
                                        materno = ?,
                                        nombre = ?,
                                        fecha_nacimiento = ?,
                                        sexo = ?,
        ...
        public function resetPassword($datos) {
            $conexion = Conexion::conectar();
            $sql = "UPDATE helpdesk.t_usuarios 
                    SET password = ? 
                    WHERE id_usuario = ?";
        ...
        public function buscarReportesUsuario($idUsuario) {
            $conexion = Conexion::conectar();
            $sql = "SELECT * FROM helpdesk.t_reportes WHERE id_usuario = '$idUsuario'";
            $respuesta = mysqli_query($conexion, $sql);
        ...
        public function buscarAsignacionPersona($idPersona) {
            $conexion = Conexion::conectar();
            $sql = "SELECT * FROM helpdesk.t_asignacion WHERE id_persona = '$idPersona'";
            $respuesta = mysqli_query($conexion, $sql);
            if(mysqli_num_rows($respuesta) > 0){
                return 1;
            } else {
        ...
        public function eliminarUsuario($datos) {
            ...
            if($reportes == 0 && $asignaciones == 0){
                ...
                $sql = "DELETE FROM helpdesk.t_usuarios WHERE id_usuario = ?";
                $query = $conexion->prepare($sql);
